...
/**
 * Base class that represents a row from the 'travelers' table.
 *
 *
 *
 * @package    propel.generator.Traveler.Base
 */
abstract class Traveler implements ActiveRecordInterface
{
    /**
     * TableMap class name
     *
     * @var string
     */
    public const TABLE_MAP = '\\Traveler\\Infrastructure\\Models\\Traveler\\Map\\TravelerTableMap';
    ...
    /**
     * @var        ObjectCollection|City[] Cross Collection to store aggregation of City objects.
     * @phpstan-var ObjectCollection&\Traversable<City> Cross Collection to store aggregation of City objects.
     */
     ...
     * @var ObjectCollection|City[]
     * @phpstan-var ObjectCollection&\Traversable<City>
     */
    ...
    /**
     * Initializes internal state of Traveler\Infrastructure\Models\Traveler\Base\Traveler object.
     */
    ...
    /**
     * Compares this with another <code>Traveler</code> instance.  If
     * <code>obj</code> is an instance of <code>Traveler</code>, delegates to
     * <code>equals(Traveler)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[TravelerTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[TravelerTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TravelerTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TravelerTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = TravelerTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Traveler\\Infrastructure\\Models\\Traveler\\Traveler'), 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildTravelerQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws \Propel\Runtime\Exception\PropelException
     * @see Traveler::setDeleted()
     * @see Traveler::isDeleted()
     */
    public function delete(?ConnectionInterface $con = null): void
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildTravelerQuery::create()
    ...
    public function save(?ConnectionInterface $con = null): int
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TravelerTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->citiesScheduledForDeletion !== null) {
                if (!$this->citiesScheduledForDeletion->isEmpty()) {
                    $pks = [];
                    foreach ($this->citiesScheduledForDeletion as $entry) {
                        $entryPk = [];
            ...
            if ($this->collCities) {
                foreach ($this->collCities as $city) {
                    if (!$city->isDeleted() && ($city->isNew() || $city->isModified())) {
                        $city->save($con);
                    }
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        $this->modifiedColumns[TravelerTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . TravelerTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(TravelerTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(TravelerTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = '`name`';
        }

        $sql = sprintf(
            'INSERT INTO `travelers` (%s) VALUES (%s)',
    ...
    {
        $pos = TravelerTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Traveler'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['Traveler'][$this->hashCode()] = true;
        $keys = TravelerTableMap::getFieldNames($keyType);
        $result = [
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
        ];
    ...
    {
        $criteria = new Criteria(TravelerTableMap::DATABASE_NAME);

        if ($this->isColumnModified(TravelerTableMap::COL_ID)) {
            $criteria->add(TravelerTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(TravelerTableMap::COL_NAME)) {
            $criteria->add(TravelerTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildTravelerQuery::create();
        $criteria->add(TravelerTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \Traveler\Infrastructure\Models\Traveler\Traveler (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Traveler\Infrastructure\Models\Traveler\Traveler Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     * If this ChildTraveler is new, it will return
    ...
    public function setTravelersCitiesRels(Collection $travelersCitiesRels, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($travelersCitiesRelsToDelete as $travelersCitiesRelRemoved) {
            $travelersCitiesRelRemoved->setTraveler(null);
        }
    ...
    protected function doAddTravelersCitiesRel(TravelersCitiesRel $travelersCitiesRel): void
    {
        ...
        $travelersCitiesRel->setTraveler($this);
    }
    ...
    public function removeTravelersCitiesRel(TravelersCitiesRel $travelersCitiesRel)
    {
        if ($this->getTravelersCitiesRels()->contains($travelersCitiesRel)) {
            ...
            $travelersCitiesRel->setTraveler(null);
        }
     ...
     * Otherwise if this Traveler is new, it will return
     * an empty collection; or if this Traveler has previously
     * been saved, it will retrieve related TravelersCitiesRels from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Traveler.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     ...
     * @phpstan-return ObjectCollection&\Traversable<TravelersCitiesRel}> List of TravelersCitiesRel objects
     */
    public function getTravelersCitiesRelsJoinCity(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = TravelersCitiesRelQuery::create(null, $criteria);
    ...
    /**
     * Initializes the collCities crossRef collection.
     *
     * By default this just sets the collCities collection to an empty collection (like clearCities());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
    ...
    public function initCities()
    {
        ...
        $this->collCities->setModel('\Traveler\Infrastructure\Models\City\City');
    }

    /**
     * Checks if the collCities collection is loaded.
     *
     * @return bool
     */
    public function isCitiesLoaded(): bool
    {
        return null !== $this->collCities;
    }

    /**
     * Gets a collection of City objects related by a many-to-many relationship
     ...
     * @param ConnectionInterface $con Optional connection object
     *
     * @return ObjectCollection|City[] List of City objects
     * @phpstan-return ObjectCollection&\Traversable<City> List of City objects
     */
    ...
    /**
     * Sets a collection of City objects related by a many-to-many relationship
     * to the current object by way of the travelers_cities cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    public function setCities(Collection $cities, ?ConnectionInterface $con = null)
    {
        ...
        $citiesScheduledForDeletion = $currentCities->diff($cities);

        foreach ($citiesScheduledForDeletion as $toDelete) {
            $this->removeCity($toDelete);
        }
    ...
    /**
     * Gets the number of City objects related by a many-to-many relationship
     * to the current object by way of the travelers_cities cross-reference table.
     *
     * @param Criteria $criteria Optional query object to filter the query
     ...
     * @param ConnectionInterface $con Optional connection object
     *
     * @return int The number of related City objects
     */
    public function countCities(?Criteria $criteria = null, $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collCitiesPartial && !$this->isNew();
        if (null === $this->collCities || null !== $criteria || $partial) {
    ...
    /**
     * Associate a City to this object
     * through the travelers_cities cross reference table.
     *
     * @param City $city
     * @return ChildTraveler The current object (for fluent API support)
     */
     ...
     * @param City $city
     */
    protected function doAddCity(City $city)
    {
        $travelersCitiesRel = new TravelersCitiesRel();

        $travelersCitiesRel->setCity($city);
        ...
        if (!$city->isTravelersLoaded()) {
            $city->initTravelers();
            $city->getTravelers()->push($this);
        } elseif (!$city->getTravelers()->contains($this)) {
            $city->getTravelers()->push($this);
        }
    ...
    public function removeCity(City $city)
    {
        if ($this->getCities()->contains($city)) {
            ...
            if ($city->isTravelersLoaded()) {
                ...
                $city->getTravelers()->removeObject($this);
            }
            ...
            $this->citiesScheduledForDeletion->push($city);
        }
    ...
    {
        return (string) $this->exportTo(TravelerTableMap::DEFAULT_STRING_FORMAT);
    }
